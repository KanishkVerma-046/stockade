...
// ── Candle generation with trend momentum ──────────────────────────────────────
...
function generateCandles(basePrice: number, count = 200): Candle[] {
  ...
  for (let i = count; i >= 0; i--) {
    // Momentum: trend persists but mean-reverts over time
    ...
    // Wicks: occasionally long, usually short
...
export default function TradingSimulator() {
  ...
  // Refs to avoid stale closures in stop-loss/take-profit checks
  ...
  // ── Live price feed with momentum-driven candles ──
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setCandles(prev => {
        ...
        // Evolve trend bias: momentum with mean reversion
        ...
        // Open a new candle every ~10 real seconds
        if (Date.now() - candleStartRef.current >= 10_000) {
          ...
          return [...prev.slice(-199), updated, newCandle];
        }
  ...
  useEffect(() => {
    ...
    if (slHit || tpHit) {
      ...
      const pnl  = mult * (currentPrice - pos.avgPrice) * pos.qty;
      setBalance(b => b + pnl);
      setTrades(t => [{
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--c-bg)] text-[var(--c-text)]">
      ...
      {/* ── Top toolbar ── */}
      <div className="flex items-center gap-3 px-3 py-2 border-b border-[var(--c-border)] bg-[var(--c-bg-soft)] overflow-x-auto">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="hidden lg:flex w-[180px] shrink-0 flex-col border-r border-[var(--c-border)] bg-[var(--c-bg-subtle)] p-3 gap-3 overflow-y-auto">
          ...
          {position.side && (
            <div className={`rounded p-2.5 border ${
              position.side === 'long'
                ? 'bg-[var(--c-green-bg)] border-[var(--c-green-dim)]'
                : 'bg-[var(--c-red-bg)] border-[var(--c-red-dim)]'
            }`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="lg:hidden flex border-b border-[var(--c-border)]">
            {(['chart', 'positions', 'trades'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-4 py-2 text-[12px] font-mono capitalize border-b-2 transition-colors ${
                  tab === t ? 'border-[#f59e0b] text-[var(--c-text)]' : 'border-transparent text-[var(--c-text-subtle)]'
                }`}>
                {t}
              </button>
            ))}
          ...
          <div className="flex-1 overflow-hidden bg-[var(--c-bg-subtle)] relative">
            ...
            {/* Price overlay */}
            <div
              className="absolute top-3 left-3 rounded px-3 py-2 backdrop-blur-sm border border-[var(--c-border)] pointer-events-none"
        ...
        {/* ── Right: Order Panel ── */}
        <div className="w-[220px] shrink-0 border-l border-[var(--c-border)] bg-[var(--c-bg-subtle)] flex flex-col overflow-y-auto">
          <div className="p-3 border-b border-[var(--c-border)]">
                ...
                <button key={t} onClick={() => setOrder(o => ({ ...o, type: t }))}
                  className={`flex-1 py-1 text-[11px] font-mono rounded capitalize transition-colors ${
                    order.type === t
                      ...
                      : 'text-[var(--c-text-subtle)] hover:text-[var(--c-text-muted)]'
                  }`}>
                  {t}
                </button>
              ))}
            ...
            <div className="flex gap-1 mb-1">
              <button
                ...
                className="flex-1 py-2 bg-[var(--c-green-bg)] border border-[var(--c-green-dim)] text-[#22c55e] text-[13px] font-mono font-semibold rounded hover:brightness-110 transition-all"
              >
                BUY <span className="text-[10px] opacity-60">[B]</span>
              </button>
              <button
                onClick={() => execOrder('sell')}
                className="flex-1 py-2 bg-[var(--c-red-bg)] border border-[var(--c-red-dim)] text-[#ef4444] text-[13px] font-mono font-semibold rounded hover:brightness-110 transition-all"
              >
          ...
          {/* Position summary */}
          <div className="p-3 border-b border-[var(--c-border)]">
            <div className="text-[11px] font-mono uppercase tracking-widest text-[var(--c-text-subtle)] mb-2">Account</div>
            {[
              { label: 'Equity', value: fmtMoney(equity),   color: equity >= 100000 ? '#22c55e' : '#ef4444' },
          ...
          <div className="p-3 flex-1 overflow-y-auto">
            <div className="text-[11px] font-mono uppercase tracking-widest text-[var(--c-text-subtle)] mb-2">Trades</div>
            {trades.length === 0 ? (
