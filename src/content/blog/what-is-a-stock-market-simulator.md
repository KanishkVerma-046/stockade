...
A simulator is a trading platform where the money and the price feed are both simulated and everything built around them works the way it really works. Be clear on the data: Stockade's prices are generated algorithmically, not pulled from an exchange, and there is no live feed behind them. What they do is *behave* like market data — proper OHLC candles with an open, high, low and close, volume that shifts from bar to bar, volatility that clusters, and the long wicks and sudden gaps that make real charts awkward to read. The order types are the real order types. The account math — cash balance, position size, unrealized profit and loss, account equity — follows the same arithmetic your broker uses. What is missing is the settlement: nobody sends your order to an exchange, and no cash leaves an account.
