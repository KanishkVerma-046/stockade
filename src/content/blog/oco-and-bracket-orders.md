...
One honest caveat, because it changes what you can learn here. Stockade's fills are frictionless — no slippage, no partial fills, no spread. That means the quantity-mismatch problem described above **cannot happen on the simulator**, so it is the one bracket failure mode you have to learn about rather than experience. Everything else — pre-committing to a ratio, resisting the urge to widen the stop, discovering how often a 2:1 target misses by a few cents — is fully available.

Run twenty bracketed trades where you set the levels first and never adjust them, then check your realized results in [the simulator](/simulator) and see what your actual win rate would need to be.
