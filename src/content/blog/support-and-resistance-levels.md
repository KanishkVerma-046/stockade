...
One caveat: every price on Stockade is generated by an algorithm, never sourced from an exchange, so the zones you find are structural practice rather than an edge on a real instrument. Fills here are frictionless too — no slippage, no spread, no partial fills — so a stop that survives on the simulator may not survive a live order. Start at [the simulator](/simulator) with $100,000 in virtual capital and treat every stop placement as an experiment.
