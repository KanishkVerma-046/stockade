...
Stockade generates each candle's volume with `Math.floor(Math.random() * 600_000 + 80_000)` — a uniformly random draw between roughly 80,000 and 680,000, independent of everything else. It is not derived from the candle's range, its direction, or its position in a trend. The expected volume on the strongest breakout candle is identical to the expected volume on a flat doji: about 380,000, every time.
