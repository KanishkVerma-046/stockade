...
Spanish (`es`), Portuguese (`pt`, pt-BR), and Japanese (`ja`) are **all complete**: the homepage, all 6 static/app pages (about, contact, markets, analytics, simulator, chart-simulator), and all 20 blog posts, in each locale. Only `/privacy`, `/terms`, `/disclaimer` (professional-review track, not started — see the design doc's rollout-strategy note before starting them) and the explicit non-goals `/404`/`/500` remain English-only. Infra also registers fr/de/ko/it but no content exists in them yet. Full architecture and per-batch history: `docs/superpowers/specs/2026-08-31-i18n-infrastructure-design.md`.

`astro.config.mjs`'s sitemap `serialize()` reads every translated-locale blog directory's (`blog-es/*.md`, `blog-pt/*.md`, `blog-ja/*.md`) `slug`/`translationOf` frontmatter directly (outside the content-layer runtime, so a small regex read stands in for `astro:content`) to inject correct `xhtml:link` hreflang alternates for blog post pairs in `sitemap.xml` — `@astrojs/sitemap`'s own i18n auto-pairing only works when a URL's path is byte-identical across locales after stripping the locale prefix, which localized slugs break by design. The locale list this reads from is the `translatedBlogLocales` array at the top of the sitemap config — add a new code there once that locale's `blog-{code}` directory exists. Any future page type with per-locale-independent slugs needs the same manual override; see the design doc's §5 caveat.

Translated blog posts live in mirrored `blogEs`/`blogPt`/`blogJa` content collections (`src/content/blog-{es,pt,ja}/*.md`, same schema as `blog` plus a required `translationOf` field, defined in `src/content.config.ts`) with their own routes at `src/pages/{locale}/blog/index.astro` and `src/pages/{locale}/blog/[slug].astro` — not a `locale` field on the English collection. **Translated posts use localized slugs** (the filename/id, e.g. `guia-de-paper-trading` in Spanish, `medias-moveis-ema-vs-sma` in Portuguese, `peipa-toreido-gaido` in Japanese, is independently chosen for the target-language keyword, not copied from English) — `translationOf` names the id of the English post being translated, and the `[slug]` routes match on that field to compute per-post hreflang alternates (up to four-way: en + whichever of es/pt/ja also have a sibling) once the sides exist. `BlogPost.astro`, `RelatedPosts.astro`, and `TableOfContents.astro` are shared across all locales and read `Astro.currentLocale` directly for their chrome text (via `ui.ts`) rather than taking a locale prop. `RelatedPosts` is deliberately locale-scoped with no minimum tag-overlap threshold and no cross-language fallback — it shows whatever other posts exist in the current locale's collection (ranked by shared tags, degrading to most-recent when none), and disappears only when there are zero others.
...
**Rollout order:** es, pt, and ja are complete site-wide except the legal pages. The legal pages (professional review, not AI translation) were deliberately deferred again in favor of ja — consistent with the same override applied when pt was picked over the legal pages. Next language was ja by registry order (`en, es, ja, fr, de, pt, ko, it`), confirmed with the user before starting rather than assumed. Next session should not assume either sequencing (which of fr/de/ko/it comes next, or the legal pages) without asking; the previous "don't re-litigate" framing has twice not survived contact with a direct user request to change course.
