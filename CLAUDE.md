...
Spanish (`es`), Portuguese (`pt`, pt-BR), Japanese (`ja`), French (`fr`), German (`de`), and Korean (`ko`) are **all complete**: the homepage, all 6 static/app pages (about, contact, markets, analytics, simulator, chart-simulator), and all 20 blog posts, in each locale. Only `/privacy`, `/terms`, `/disclaimer` (professional-review track, not started — see the design doc's rollout-strategy note before starting them) and the explicit non-goals `/404`/`/500` remain English-only. Infra also registers `it` but no content exists in it yet. Full architecture and per-batch history: `docs/superpowers/specs/2026-08-31-i18n-infrastructure-design.md`.

`astro.config.mjs`'s sitemap `serialize()` reads every translated-locale blog directory's (`blog-es/*.md`, `blog-pt/*.md`, `blog-ja/*.md`, `blog-fr/*.md`, `blog-de/*.md`, `blog-ko/*.md`) `slug`/`translationOf` frontmatter directly (outside the content-layer runtime, so a small regex read stands in for `astro:content`) to inject correct `xhtml:link` hreflang alternates for blog post pairs in `sitemap.xml` — `@astrojs/sitemap`'s own i18n auto-pairing only works when a URL's path is byte-identical across locales after stripping the locale prefix, which localized slugs break by design. The locale list this reads from is the `translatedBlogLocales` array at the top of the sitemap config — add a new code there once that locale's `blog-{code}` directory exists. Any future page type with per-locale-independent slugs needs the same manual override; see the design doc's §5 caveat.

Translated blog posts live in mirrored `blogEs`/`blogPt`/`blogJa`/`blogFr`/`blogDe`/`blogKo` content collections (`src/content/blog-{es,pt,ja,fr,de,ko}/*.md`, same schema as `blog` plus a required `translationOf` field, defined in `src/content.config.ts`) with their own routes at `src/pages/{locale}/blog/index.astro` and `src/pages/{locale}/blog/[slug].astro` — not a `locale` field on the English collection. **Translated posts use localized slugs** (the filename/id, e.g. `guia-de-paper-trading` in Spanish, `medias-moveis-ema-vs-sma` in Portuguese, `peipa-toreido-gaido` in Japanese, `guide-du-paper-trading` in French, `paper-trading-leitfaden` in German, `moeuituja-gaideu` in Korean, is independently chosen for the target-language keyword, not copied from English) — `translationOf` names the id of the English post being translated, and the `[slug]` routes match on that field to compute per-post hreflang alternates (up to seven-way: en + whichever of es/pt/ja/fr/de/ko also have a sibling) once the sides exist. `BlogPost.astro`, `RelatedPosts.astro`, and `TableOfContents.astro` are shared across all locales and read `Astro.currentLocale` directly for their chrome text (via `ui.ts`) rather than taking a locale prop. `RelatedPosts` is deliberately locale-scoped with no minimum tag-overlap threshold and no cross-language fallback — it shows whatever other posts exist in the current locale's collection (ranked by shared tags, degrading to most-recent when none), and disappears only when there are zero others.
...
5. Sweep existing `/{locale}/*` pages for in-body prose links to this page's English URL that should now point at the new translated sibling instead. Note the established exception: cross-post links *inside blog post body content* stay pointed at the English URL even when a translated sibling exists (checked against `blog-es`/`blog-pt`/`blog-fr`/`blog-de`/`blog-ko` source — every in-body post-to-post link does this); only page-level chrome links (nav, "how it works" sections, FAQ) get localized. This includes each locale's own blog `[slug].astro` route (and the English one) — its `getStaticPaths` must enumerate every other translated collection to compute reciprocal alternates, and it's easy to add a new locale's own route while forgetting to add it to the *existing* routes' alternate lookups; the de rollout found and fixed exactly that gap left over from the fr rollout, and the ko rollout correctly updated all six existing `[slug].astro` routes (`en`, `es`, `pt`, `ja`, `fr`, `de`) when it landed.

**Rollout order:** es, pt, ja, fr, de, and ko are complete site-wide except the legal pages. The legal pages (professional review, not AI translation) have now been deferred five times in favor of a new language (pt over legal, ja over legal, fr over legal, de over legal, ko over legal) — a consistent pattern, not a one-off. Next language was ko by registry order (`en, es, ja, fr, de, pt, ko, it`) — the user again gave explicit standing direction to proceed straight through registry order without a per-language confirmation prompt (as with the fr and de rollouts), plus explicit instructions to do the whole thing in one pass, keep only one subagent active at a time, and not deploy. The only locale left in the registry with no content at all is `it`. Absent a similar explicit instruction in a future session, go back to asking before assuming it's next, or whether it's time for the legal pages instead.
