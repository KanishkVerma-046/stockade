...
Spanish (`es`), Portuguese (`pt`, pt-BR), Japanese (`ja`), French (`fr`), German (`de`), Korean (`ko`), and Italian (`it`) are **all complete**: the homepage, all 6 static/app pages (about, contact, markets, analytics, simulator, chart-simulator), the legal pages (privacy, terms, disclaimer), and all 20 blog posts, in each locale. Only the explicit non-goals `/404`/`/500` remain English-only. `it` was the last locale registered with no content — the registry (`en, es, ja, fr, de, pt, ko, it`) is now fully translated site-wide, including legal. Full architecture and per-batch history: `docs/superpowers/specs/2026-08-31-i18n-infrastructure-design.md`.

**Legal pages (`/privacy`, `/terms`, `/disclaimer`) were AI-translated on 2026-09-04/05**, overriding the earlier "professional-review track" decision recorded in the design doc's Phase 3 addenda — the user explicitly chose AI translation (the same process used for every other page) plus a visible in-page notice, instead of the original plan (professional review, no notice). Each translated legal page renders a `LegalTranslationNotice` component (`src/components/layout/LegalTranslationNotice.astro`, `legal.translationNotice`/`legal.viewEnglish` keys in `ui.ts`) immediately after the header block — before the disclaimer page's amber warning box — stating the translation is for convenience and linking back to the English original, which is authoritative. `PRIVACY_ALTERNATES`/`TERMS_ALTERNATES`/`DISCLAIMER_ALTERNATES` were added to `alternates.ts`'s `REGISTRY` following the standard checklist below, so `Footer`'s legal links and `CookieConsent`'s privacy link (previously hardcoded to the English `/privacy/` — see the design doc's Phase 2 note, now stale) both resolve per-locale automatically.

`astro.config.mjs`'s sitemap `serialize()` reads every translated-locale blog directory's (`blog-es/*.md`, `blog-pt/*.md`, `blog-ja/*.md`, `blog-fr/*.md`, `blog-de/*.md`, `blog-ko/*.md`, `blog-it/*.md`) `slug`/`translationOf` frontmatter directly (outside the content-layer runtime, so a small regex read stands in for `astro:content`) to inject correct `xhtml:link` hreflang alternates for blog post pairs in `sitemap.xml` — `@astrojs/sitemap`'s own i18n auto-pairing only works when a URL's path is byte-identical across locales after stripping the locale prefix, which localized slugs break by design. The locale list this reads from is the `translatedBlogLocales` array at the top of the sitemap config — add a new code there once that locale's `blog-{code}` directory exists. Any future page type with per-locale-independent slugs needs the same manual override; see the design doc's §5 caveat.

Translated blog posts live in mirrored `blogEs`/`blogPt`/`blogJa`/`blogFr`/`blogDe`/`blogKo`/`blogIt` content collections (`src/content/blog-{es,pt,ja,fr,de,ko,it}/*.md`, same schema as `blog` plus a required `translationOf` field, defined in `src/content.config.ts`) with their own routes at `src/pages/{locale}/blog/index.astro` and `src/pages/{locale}/blog/[slug].astro` — not a `locale` field on the English collection. **Translated posts use localized slugs** (the filename/id, e.g. `guia-de-paper-trading` in Spanish, `medias-moveis-ema-vs-sma` in Portuguese, `peipa-toreido-gaido` in Japanese, `guide-du-paper-trading` in French, `paper-trading-leitfaden` in German, `moeuituja-gaideu` in Korean, `guida-al-paper-trading` in Italian, is independently chosen for the target-language keyword, not copied from English) — `translationOf` names the id of the English post being translated, and the `[slug]` routes match on that field to compute per-post hreflang alternates (up to eight-way: en + whichever of es/pt/ja/fr/de/ko/it also have a sibling) once the sides exist. `BlogPost.astro`, `RelatedPosts.astro`, and `TableOfContents.astro` are shared across all locales and read `Astro.currentLocale` directly for their chrome text (via `ui.ts`) rather than taking a locale prop. `RelatedPosts` is deliberately locale-scoped with no minimum tag-overlap threshold and no cross-language fallback — it shows whatever other posts exist in the current locale's collection (ranked by shared tags, degrading to most-recent when none), and disappears only when there are zero others.
...
4. Add the `alternates={…}` prop to the existing English page's `<Layout>`/`<AppLayout>` too, so hreflang/switcher are reciprocal. In practice this is automatic once step 1's constant carries every live locale's path — existing locale pages don't need per-page edits when a new locale is added, since they import the same shared constant.
5. Sweep existing `/{locale}/*` pages for in-body prose links to this page's English URL that should now point at the new translated sibling instead. Note the established exception: cross-post links *inside blog post body content* stay pointed at the English URL even when a translated sibling exists (checked against `blog-es`/`blog-pt`/`blog-fr`/`blog-de`/`blog-ko`/`blog-it` source — every in-body post-to-post link does this); only page-level chrome links (nav, "how it works" sections, FAQ) get localized. This includes each locale's own blog `[slug].astro` route (and the English one) — its `getStaticPaths` must enumerate every other translated collection to compute reciprocal alternates, and it's easy to add a new locale's own route while forgetting to add it to the *existing* routes' alternate lookups; the de rollout found and fixed exactly that gap left over from the fr rollout, the ko rollout correctly updated all six existing `[slug].astro` routes (`en`, `es`, `pt`, `ja`, `fr`, `de`) when it landed, and the it rollout did the same for all seven (`en`, `es`, `pt`, `ja`, `fr`, `de`, `ko`).

**Rollout order:** es, pt, ja, fr, de, ko, and it are now complete site-wide, including the legal pages — every locale in the registry (`en, es, ja, fr, de, pt, ko, it`) has complete content everywhere except the `/404`/`/500` non-goals. The legal-page translation batch (2026-09-04/05) ran on explicit standing direction: AI-translate all three legal pages into all seven non-English locales in one pass (with the added translation notice, per the override above), straight through without a per-locale confirmation prompt, one subagent at a time, verified against the built `dist/` output after each locale, and no deploy. There is no work left in the registry to roll out automatically — the next i18n session should ask whether to add a new locale to the registry, since every existing locale and page type is now fully translated.
