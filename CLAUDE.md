...
Spanish (`es`), Portuguese (`pt`, pt-BR), Japanese (`ja`), French (`fr`), German (`de`), Korean (`ko`), and Italian (`it`) are **all complete**: the homepage, all 6 static/app pages (about, contact, markets, analytics, simulator, chart-simulator), and all 20 blog posts, in each locale. Only `/privacy`, `/terms`, `/disclaimer` (professional-review track, not started — see the design doc's rollout-strategy note before starting them) and the explicit non-goals `/404`/`/500` remain English-only. `it` was the last locale registered with no content — the registry (`en, es, ja, fr, de, pt, ko, it`) is now fully translated site-wide. Full architecture and per-batch history: `docs/superpowers/specs/2026-08-31-i18n-infrastructure-design.md`.
...
**Rollout order:** es, pt, ja, fr, de, ko, and it are complete site-wide except the legal pages. The legal pages (professional review, not AI translation) have now been deferred six times in favor of a new language (pt over legal, ja over legal, fr over legal, de over legal, ko over legal, it over legal) — a consistent pattern, not a one-off. Registry order (`en, es, ja, fr, de, pt, ko, it`) is now fully translated — every locale in the registry has complete site-wide content except the legal pages. The it rollout (like fr, de, and ko before it) ran on explicit standing direction: proceed straight through registry order without a per-language confirmation prompt, do the whole thing in one pass, keep only one subagent active at a time, and don't deploy. There is no next locale left in the registry to roll out automatically — the next i18n session should ask whether to add a new locale to the registry or finally tackle the legal pages, rather than assuming either.
